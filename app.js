...
function initEditorPage() {
  ...
  function inferWritingMode(prompt = '', selectedMode = 'auto') {
    ...
    if (/continue|keep writing|carry on/i.test(text)) return 'continue';
  ...
  function insertGeneratedIntoEditor(replace = false) {
    ...
    const generatedTitle = latestGeneratedKind === 'outline' ? 'Generated Outline' : 'AI Writing Draft';
  ...
  async function generateWriting() {
    ...
    try {
      const markdown = await requestAssistant('generate_writing', {
        ...
        title: titleInput?.value.trim() || '',
        author: authorInput?.value.trim() || '',
        currentContent: utils.stripHtml(quill.root.innerHTML).slice(0, 5000)
      });
